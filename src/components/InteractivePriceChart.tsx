...
export default function InteractivePriceChart({ 
  ...
  const [targetOec, setTargetOec] = useState<'revenue' | 'profit'>(propTargetOec === 'conversion' ? 'profit' : propTargetOec || 'profit');
  ...
  const updateComparison = useCallback(() => {
    ...
    console.log('Updating optimal price:', optimal, 'targetOec:', targetOec);
  ...
  const setToOptimalPrice = () => {
    if (optimalPrice !== null) {
      setPriceB(optimalPrice);
    }
  };
  ...
  const getOptimalMetrics = (): ComparisonPoint | null => {
    ...
    // Calculate using price elasticity
        ...
        {optimalMetrics && (
          <div className="text-center p-4 border border-red-700 rounded-lg">
            ...
            <div className="text-xs text-gray-500 mt-1">
              {targetOec === 'profit' ? 'Profit Change' : 'Revenue Change'}
            </div>
          </div>
        )}
